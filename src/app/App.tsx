...
function DiagnosisSection() {
  const { step, currentQuestion, answers, setAnswer, goNext, scrollToQuiz, registerScrollRef } = useDiagnosis();
...
function SampleRequestModal() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    // Phase 1에서 실제 API 호출 — 현재는 UI 완성 시연용
...
function DiagnosisResult() {
  const { recommendedProducts, selectedBenefit, reset } = useDiagnosis();
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-sm border border-[#1A7F5A]/10">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button className="flex-1 py-4 bg-[#FEE500] text-[#371D1E] rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-[#f4db00] transition-colors">
              <MessageCircle className="w-5 h-5 fill-current" /> 카카오페이로 구매
            </button>
            <button className="flex-1 py-4 bg-[#1A7F5A] text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-[#146648] transition-colors">
              제품 더 보기 <ChevronRight className="w-5 h-5" />
            </button>
          </div>
        </div>
